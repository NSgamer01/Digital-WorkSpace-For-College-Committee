// ============================================
// config/committeeDbManager.js — Multi-Pool Manager
// ============================================
...
class CommitteeDbManager {
    ...
    getPool(slug) {
        ...
        if (!dbName) {
            throw new Error(
                `Unknown committee slug: "${slug}". Valid slugs: ${this.getRegisteredSlugs().join(', ')}`
            );
        }
        ...
        pool.on('connect', () => {
            console.log(`  🟢 Committee DB [${normalizedSlug}]: client connected to ${dbName}`);
        });

        pool.on('error', (err) => {
            console.error(`  ❌ Committee DB [${normalizedSlug}]: unexpected error —`, err.message);
        });
